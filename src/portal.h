...
#include "execution/executor_block_nlj.h"
...
struct PortalStmt {
    ...
    int limit_count = 0;  // 0 means no limit
    ...
    PortalStmt(portalTag tag_, std::vector<TabCol> sel_cols_, std::unique_ptr<AbstractExecutor> root_, std::shared_ptr<Plan> plan_, int limit) :
            tag(tag_), sel_cols(std::move(sel_cols_)), root(std::move(root_)), plan(std::move(plan_)), limit_count(limit) {}
};
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if(auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)){
            ...
            return std::make_unique<AggregationExecutor>(convert_plan_executor(x->subplan_, context),
                                                         x->agg_type_, x->agg_col_,
                                                         x->alias_.empty() ? x->src_col_ : x->alias_);
        } else if(auto x = std::dynamic_pointer_cast<SortPlan>(plan)) {
            auto sort_exec = std::make_unique<SortExecutor>(convert_plan_executor(x->subplan_, context),
                                            x->sel_cols_, x->is_descs_);
            sort_exec->limit_ = x->limit_count_ > 0 ? (size_t)x->limit_count_ : SIZE_MAX;
            return sort_exec;
        }
        return nullptr;
    }
