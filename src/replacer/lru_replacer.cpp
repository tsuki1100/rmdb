...
bool LRUReplacer::victim(frame_id_t* frame_id) {
    ...
    // LRUlist_ back is the least recently used (unpinned longest ago)
...
void LRUReplacer::unpin(frame_id_t frame_id) {
    std::scoped_lock lock{latch_};
    // Only add if not already in the replacer and not at capacity
    if (LRUhash_.find(frame_id) == LRUhash_.end() && LRUlist_.size() < max_size_) {
