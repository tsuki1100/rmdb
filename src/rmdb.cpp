...
static inline int socket_close(int fd) { return closesocket(fd); }
#else
#include <netinet/in.h>
static inline int socket_close(int fd) { return close(fd); }
#endif
#include <atomic>
...
void *client_handler(void *sock_fd) {
    ...
    socket_close(fd);           // close a file descriptor.
...
void start_server() {
    ...
    int val = 1;
    setsockopt(sockfd_server, SOL_SOCKET, SO_REUSEADDR, (const char*)&val, sizeof(val));
