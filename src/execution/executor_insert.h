    ...
    std::unique_ptr<RmRecord> Next() override {
        ...
        for (size_t i = 0; i < values_.size(); i++) {
            ...
            if (col.type != val.type) {
                // 允许 INT -> BIGINT 和 STRING -> DATETIME 隐式转换
                if (col.type == TYPE_DATETIME && val.type == TYPE_STRING) {
                    int64_t dt;
                    if (!parse_datetime(val.str_val, dt)) {
                        throw InternalError("Invalid datetime value");
                    }
                    values_[i] = Value();
                    values_[i].set_datetime(dt);
                } else if (col.type == TYPE_BIGINT && val.type == TYPE_INT) {
                    int64_t bigint_val = val.int_val;
                    values_[i] = Value();
                    values_[i].set_bigint(bigint_val);
                } else {
        ...
        // Record in transaction write set for rollback + WAL log
        if (context_ != nullptr && context_->txn_ != nullptr) {
            context_->txn_->append_write_record(
                new WriteRecord(WType::INSERT_TUPLE, tab_name_, rid_, rec));
            // WAL: log the insert
