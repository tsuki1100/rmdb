...
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    ...
    // Collect all conditions on this table
    std::map<std::string, const Condition*> cond_map;
    for (auto& cond : curr_conds) {
        if (cond.is_rhs_val && cond.lhs_col.tab_name == tab_name) {
            cond_map[cond.lhs_col.col_name] = &cond;
        }
    }
    if (cond_map.empty()) return false;
    // Try each index, match by leftmost prefix
    // For leftmost prefix matching: the first K columns must all have conditions (any type)
    for (auto& index : tab.indexes) {
        std::vector<std::string> matched;
        bool matches = true;
        for (auto& col : index.cols) {
            auto it = cond_map.find(col.name);
            if (it != cond_map.end()) {
                matched.push_back(col.name);
            } else {
                break; // leftmost prefix stops here
            }
        }
        if (!matched.empty() && matched.size() > index_col_names.size()) {
            index_col_names = matched;
        }
    }
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    // Also handle LIMIT if no ORDER BY (LIMIT only, sort by first column)
    std::vector<TabCol> sel_cols;
    std::vector<bool> is_descs;

    if (x->order && !x->order->order_cols.empty()) {
        std::vector<std::string> tables = query->tables;
        ...
        for (auto &oc : x->order->order_cols) {
            TabCol sel_col;
            for (auto &col : all_cols) {
                if (col.name == oc.col->col_name) {
                    sel_col = {.tab_name = col.tab_name, .col_name = col.name};
                    break;
                }
            }
            sel_cols.push_back(sel_col);
            is_descs.push_back(oc.dir == ast::OrderBy_DESC);
        }
        int limit = x->has_limit ? x->limit_count : 0;
        return std::make_shared<SortPlan>(T_Sort, std::move(plan), sel_cols, is_descs, limit);
    }
    return plan;
}
...
std::shared_ptr<Plan> Planner::generate_select_plan(std::shared_ptr<Query> query, Context *context) {
    ...
    if (query->has_agg) {
        // Insert aggregation plan between base plan and projection
        plannerRoot = std::make_shared<AggregationPlan>(T_Aggregation, std::move(plannerRoot),
                                                         query->agg_type, query->agg_col,
                                                         query->agg_src_col, query->agg_alias);
    }
