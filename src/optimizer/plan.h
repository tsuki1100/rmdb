...
{
    public:
        SortPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<TabCol> sel_cols, std::vector<bool> is_descs, int limit = 0)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            sel_cols_ = std::move(sel_cols);
...
{
    public:
        AggregationPlan(PlanTag tag, std::shared_ptr<Plan> subplan, AggType agg_type,
                       TabCol agg_col, std::string src_col, std::string alias)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            agg_type_ = agg_type;
            agg_col_ = std::move(agg_col);
            src_col_ = std::move(src_col);
