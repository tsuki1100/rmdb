...
enum SvType {
    SV_TYPE_INT, SV_TYPE_FLOAT, SV_TYPE_BIGINT, SV_TYPE_DATETIME, SV_TYPE_STRING
};
...
struct IntLit : public Value {
    ...
    IntLit(int64_t val_, bool overflow_) : val(val_), overflow(overflow_) {}
};
...
struct AggFn : public TreeNode {
    AggType agg_type;
    std::string col_name;    // column to aggregate (empty for COUNT(*))
    std::string alias;        // AS alias name

    AggFn(AggType agg_type_, std::string col_name_, std::string alias_ = "")
...
struct OrderCol {
    ...
    OrderByDir dir;
    OrderCol(std::shared_ptr<Col> col_, OrderByDir dir_) : col(std::move(col_)), dir(dir_) {}
};
...
struct SemValue {
    ...
    std::shared_ptr<JoinClause> sv_join;

    std::shared_ptr<AggFn> sv_agg;
};
