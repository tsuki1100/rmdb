...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (slot_no == file_hdr_.num_records_per_page) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        throw InternalError("insert_record: no free slot in page");
    }
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if (page == nullptr) {
        throw InternalError("fetch_page_handle: failed to fetch page");
    }
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    PageId page_id{fd_, (page_id_t)disk_manager_->get_fd2pageno(fd_)};
    Page* page = buffer_pool_manager_->new_page(&page_id);
    if (page == nullptr) {
        throw InternalError("create_new_page_handle: failed to allocate new page");
    }
